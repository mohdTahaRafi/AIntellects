...
def process_frame(frame, lane_id):
    ...
    # light_color = (0, 0, 255)  # Default red
    # light_status = "RED"
    # cv2.circle(frame, (width - 50, 30), 20, light_color, -1)
    # cv2.putText(frame, light_status, (width - 80, 70), 
    #             cv2.FONT_HERSHEY_SIMPLEX, 0.7, light_color, 2)
    
    return frame, vehicle_count, traffic_density
...
def send_traffic_data(data):
    url = "http://localhost:5000/traffic/data"
    headers = {"Content-Type": "application/json"}
    response = requests.post(url, data=json.dumps(data), headers=headers)
    if response.status_code == 201:
        print("Traffic data sent successfully")
    else:
        print(f"Failed to send traffic data: {response.status_code}")

def main():
    # Initialize video captures
    caps = [cv2.VideoCapture('sample.mp4'), cv2.VideoCapture('sample.mp4'), cv2.VideoCapture('sample.mp4'), cv2.VideoCapture('sample.mp4')]
    ...
    try:
        while True:  # Main cycle loop
            ...
            for i in range(4):
                ...
                _, _, density = process_frame(frame, i+1)
            ...
            while remaining_lanes:  # Phase loop within cycle
                ...
                while time.time() - start_time < green_duration:
                    ...
                    for i in range(4):
                        ...
                        frame = cv2.resize(frame, (width, height))
                        processed_frame, _, density = process_frame(frame, i+1)
                        current_densities[i] = density
